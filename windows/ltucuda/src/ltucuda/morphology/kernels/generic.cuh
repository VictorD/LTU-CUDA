#ifndef _GENERIC_CUDA_KERNEL_H_
#define _GENERIC_CUDA_KERNEL_H_
...
__device__ /*inline*/ dataType _processPixel(const int x, const int y, const dataType *img, int imgStep, morphMask mask) {
	...
	int imgOffset = y*imgStep + x;
    for (h = 0; h < mask.height; ++h) {
        for (w = 0; w < mask.width; ++w) {
            if (mask.data[h*mask.pitch+w] > 0) {
                if (MOP == ERODE) {
                    newval = img[imgOffset + (h*imgStep + w)];//- surfaceHeights[maskPos]; TODO: Update this device pointer.
                    if (newval < result)
                        result = newval;
                } else {
...
template <class dataType, morphOperation MOP> 
__global__ void genericKernel(const dataType *img, int imgStep, dataType *result, int resultStep, unsigned int width, 
...
int _globalGeneric(const dataType *img, int imgStep, dataType *result, int resultStep, rect2d roi, morphMask mask, rect2d border) {
    ...
    genericKernel<dataType, MOP><<<gridSize,blockSize>>>(img, imgStep, result, resultStep, width, height, mask);
#if 1
